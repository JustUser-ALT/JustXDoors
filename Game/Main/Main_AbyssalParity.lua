...
local function applyNoclip()
    ...
    local enabled = Elements.NoclipToggle and Elements.NoclipToggle.Value == true

    -- DOORS uses dedicated collision objects in addition to HumanoidRootPart.
    -- Abyssal disables the character's physical collision every frame because
    -- the game can restore CanCollide on the next physics step.
    for _, obj in ipairs(Character:GetDescendants()) do
        if obj:IsA("BasePart") then
            local name = obj.Name
            if name == "HumanoidRootPart"
                or name == "Collision"
                or name == "CollisionPart"
                or name == "CollisionClone"
                or name == "CollisionPartClone"
                or obj.Parent == Character then
                pcall(function()
    ...
    if RootPart then
        pcall(function() RootPart.CanCollide = not enabled end)
    end
end
