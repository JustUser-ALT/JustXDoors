...
-- Noclip — DOORS uses Collision + CollisionClone, not RootPart.CanCollide
-- Abyssal sets Collision.CanCollide = false every frame and uses
-- CollisionClone.CanCollide to control actual solid collision.
local Collision      = nil   -- Character.Collision (server-placed hitbox)
local CollisionClone = nil   -- cloned by us on CharacterAdded
...
------------------------------------------------------
-- NOCLIP
-- DOORS uses a server-placed "Collision" part + our "CollisionClone"
-- as the real physics hitbox, exactly like Abyssal.
-- We set Collision.CanCollide = false every frame (server resets it)
-- and gate CollisionClone.CanCollide on the toggle value.
------------------------------------------------------

local function setupCollision()
    if not Character then return end

    -- Grab (or wait) for the server-placed Collision part
    Collision = Character:FindFirstChild("Collision")
               or Character:WaitForChild("Collision", 5)

    if not Collision then return end

    -- Destroy old clone if present
    local old = Character:FindFirstChild("CollisionClone")
...
local function applyNoclip()
    ...
    -- Always keep Collision (server hitbox) non-colliding —
    -- the server resets this so we enforce it every Heartbeat.
    if Collision and Collision.Parent then
        pcall(function() Collision.CanCollide = false end)
        -- Also kill the CollisionCrouch child if present
        local cc = Collision:FindFirstChild("CollisionCrouch")
        if cc then pcall(function() cc.CanCollide = false end) end
    end

    -- CollisionClone controls whether the player clips or not
    if CollisionClone and CollisionClone.Parent then
        pcall(function()
            CollisionClone.CanCollide = not noClip
            local ccClone = CollisionClone:FindFirstChild("CollisionCrouch")
            if ccClone then ccClone.CanCollide = not noClip end
        end)
    end

    -- RootPart fallback (OldHotel / Fools floors use it)
    if RootPart then
        pcall(function() RootPart.CanCollide = not noClip end)
    end
end
