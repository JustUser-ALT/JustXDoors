...
local function getRoomAmbient()
    ...
    return Services and Services.Lighting and Services.Lighting.Ambient or Color3.fromRGB(255,255,255)
end

local function applyAmbient(enabled)
    if not Services or not Services.Lighting or not Services.TweenService then return end
    local targetColor
    if enabled then
        targetColor = Elements.AmbientColor and Elements.AmbientColor.Value or Color3.fromRGB(255,255,255)
    else
        targetColor = getRoomAmbient()
    end
    pcall(function()
        Services.TweenService:Create(Services.Lighting, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), { Ambient = targetColor }):Play()
    end)
end
...
local function applyHidingTransparency(enabled, alpha)
    local lp = getLocalPlayer()
    if not lp or not Services or not Services.TweenService then return end
    ...
    for _, obj in ipairs(HidingSpots) do
        if obj and obj.Parent then
            ...
            local isHiding = hiddenPlayer and hiddenPlayer.Value == lp
            for _, part in ipairs(obj:GetDescendants()) do
                if part:IsA("BasePart") then
                    ...
                    if old ~= nil then
                        ...
                        pcall(function()
                            Services.TweenService:Create(part, TweenInfo.new(0.25, Enum.EasingStyle.Linear), { Transparency = target }):Play()
                        end)
                    end
                end
            end
...
local function handleHidingTransparency(obj)
    ...
    for _, part in ipairs(obj:GetDescendants()) do
        if part:IsA("BasePart") and part:GetAttribute("Transparency_Old") == nil then
            part:SetAttribute("Transparency_Old", part.Transparency)
        end
    end
    if not table.find(HidingSpots, obj) then table.insert(HidingSpots, obj) end

    local hiddenPlayer = obj:FindFirstChild("HiddenPlayer", true)
    if hiddenPlayer and not hiddenPlayer:GetAttribute("JXD_HidingWatcher") then
        hiddenPlayer:SetAttribute("JXD_HidingWatcher", true)
        local ok, conn = pcall(function()
            return hiddenPlayer:GetPropertyChangedSignal("Value"):Connect(function()
                local enabled = Elements.TransparentHidingSpotsToggle and Elements.TransparentHidingSpotsToggle.Value == true
...
local function scanHidingSpots(root)
    ...
    for _, obj in ipairs(root:GetDescendants()) do
        ...
        if string.find(name, "hidingspot", 1, true) or obj:FindFirstChild("HidingPrompt") or obj:FindFirstChild("HidePrompt") then
...
function Visual:Connect()
    ...
    if CurrentRooms then
        ...
        local ok, conn = pcall(function()
            return CurrentRooms.ChildAdded:Connect(function(room)
                task.defer(function() scanHidingSpots(room) end)
            end)
        end)
        if ok and conn then table.insert(LocalConnections, conn) end
    end
    ...
    Connections:Connect(
        ...
        function()
            ...
            -- DOORS continuously restores room lighting. Abyssal therefore
            -- reapplies the selected Ambient while the toggle is enabled.
            if Elements.AmbientToggle and Elements.AmbientToggle.Value
                and Elements.AmbientColor and Services.Lighting then
                local desired = Elements.AmbientColor.Value
                if Services.Lighting.Ambient ~= desired then
...
function Visual:Build()
    ...
    -- Create Visual on the RIGHT column, beside Character.
    local leftTabbox = UI:AddTabbox(Tab, "Right", "Visual")
