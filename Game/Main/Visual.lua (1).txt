...
-- Snapshot original Ambient before we touch it
local OldAmbient = nil

local function snapshotAmbient()
    if Services and Services.Lighting then
        OldAmbient = Services.Lighting.Ambient
    end
end
...
local function applyAmbient(enabled)
    ...
    if enabled then
        ...
        -- Try room attribute first, fallback to our snapshot
        ...
        pcall(function()
            ...
            if CurrentRooms and lp then
                ...
                if roomFolder then
                    local attr = roomFolder:GetAttribute("Ambient")
                    if attr then
                        targetColor = attr
                        restored    = true
                    end
        ...
        if not restored then
            targetColor = OldAmbient or Services.Lighting.Ambient
        end
    end
...
-- We also watch DescendantAdded so parts that replicate late get snapshotted.
local function handleHidingTransparency(obj)
    ...
    -- Watch for late-replicating parts
    pcall(function()
        obj.DescendantAdded:Connect(function(d)
            task.defer(function() snapshotPart(d) end)
        end)
    end)
end
...
function Visual:Init(CoreModules)
    ...
    -- Snapshot lighting state before any toggles run
...
function Visual:Build()
    ...
    -- Right-side tabbox so it sits beside the Character groupbox
    local rightTabbox = UI:AddRightTabbox(Tab, "Visual")
    if not rightTabbox then
