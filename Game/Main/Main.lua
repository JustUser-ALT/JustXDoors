...
function Main:Init(CoreModules)
    ...
    -- Load Visual lazily after Core is ready.
    -- This prevents an internal Visual module error from breaking Main.
    do
        local ok, result = pcall(function()
            return require("Game/Main/Visual")
        end)

        if ok and type(result) == "table" then
            Visual = result

            if type(Visual.Init) == "function" then
                local initOk, initErr = pcall(function()
                    Visual:Init(CoreModules)
                end)

                if not initOk then
                    warn("[JustXDoors GameMain] Visual Init failed: " .. tostring(initErr))
                    Visual = nil
                end
            end
        else
            warn("[JustXDoors GameMain] Visual module could not be loaded: " .. tostring(result))
            Visual = nil
        end
    end

    -- Load General lazily after Core is ready.
    do
        local ok, result = pcall(function()
            return require("Game/Main/General")
        end)

        if ok and type(result) == "table" then
            General = result

            if type(General.Init) == "function" then
                local initOk, initErr = pcall(function()
                    General:Init(CoreModules)
                end)

                if not initOk then
                    warn("[JustXDoors GameMain] General Init failed: " .. tostring(initErr))
                    General = nil
                end
            end
        else
            warn("[JustXDoors GameMain] General module could not be loaded: " .. tostring(result))
            General = nil
        end
    end
...
function Main:Build()
    ...
    -- Visual is optional: if it fails, Character/Main still starts.
    if Visual then
        if type(Visual.Build) == "function" then
            local ok, err = pcall(function()
                Visual:Build()
            end)

            if not ok then
                warn("[JustXDoors GameMain] Visual Build failed: " .. tostring(err))
            end
        else
            warn("[JustXDoors GameMain] Visual loaded, but Build() is missing.")
        end
    else
        warn("[JustXDoors GameMain] Visual is nil; Visual UI was not loaded.")
    end

    -- General is optional: if it fails, Character/Visual still start.
    if General then
        if type(General.Build) == "function" then
            local ok, err = pcall(function()
                General:Build()
            end)

            if not ok then
                warn("[JustXDoors GameMain] General Build failed: " .. tostring(err))
            end
        else
            warn("[JustXDoors GameMain] General loaded, but Build() is missing.")
        end
    else
        warn("[JustXDoors GameMain] General is nil; General UI was not loaded.")
    end

    -- Call through the module table instead of relying on the Build caller's self.
    Main:Connect()
...
function Main:Destroy()
    if General and type(General.Destroy) == "function" then
        pcall(function()
            General:Destroy()
        end)
    end
    ...
    if Visual and type(Visual.Destroy) == "function" then
        pcall(function()
            Visual:Destroy()
        end)
    end
